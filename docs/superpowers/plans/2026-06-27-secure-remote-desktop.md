...
```bash
cd /Users/example/CodePilot/ios
python3 scripts/dke_ota_build.py build --app codexphone --branch release
```

Expected: status reaches `complete`; output reports bundle ID `legacy.bundle.identifier`; both public manifest and IPA return HTTP 200; the OTA install updates the configured CodePilot app rather than creating another instance.
