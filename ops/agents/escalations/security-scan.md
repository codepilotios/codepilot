...
- A redacted rescan covered all 278 commits reachable from local refs and found private-identifier matches in many historical commit trees. Commit metadata includes non-public identity or email metadata on a small subset of commits. The current tree passes the privacy audit, and a redacted history scan found no recognized live-secret pattern outside audit fixtures, but the repository history must still be treated as unsanitized. Coordinate a history-and-metadata rewrite plus clone migration before representing it as safe; never paste historical values into an issue or pull request.
