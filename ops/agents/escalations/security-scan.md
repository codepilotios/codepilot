...
- A redacted rescan covered all 275 reachable commits and found private-identifier matches in 259 historical commit trees. Commit metadata includes non-public identity or email metadata on a small subset of commits. The current tree passes the privacy audit and recognized live-secret checks, but the repository history must still be treated as unsanitized. Coordinate a history-and-metadata rewrite plus clone migration before representing it as safe; never paste historical values into an issue or pull request.
