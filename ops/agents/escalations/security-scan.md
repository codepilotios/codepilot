...
- A redacted rescan covered all 259 reachable commits and found generic private-identifier matches in 240 historical commit trees. Commit metadata includes four distinct identities, with non-public identity or email metadata on a small subset of commits. The current tree passes the privacy audit and recognized live-secret checks, but the repository history must still be treated as unsanitized. Coordinate a history-and-metadata rewrite plus clone migration before representing it as safe; never paste historical values into an issue or pull request.
