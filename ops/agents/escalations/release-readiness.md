...
Status: CodePilot is not ready for a new OTA, TestFlight, or App Store release. Local unsigned builds, gateway tests, and public CI are healthy, but release identity, signing/App Store Connect access, privacy work, screenshots, and review metadata remain blocked.
...
- App Store Connect inspection was unavailable in this run. The Apple developer team setting is empty, and no App Store Connect API credentials or app ID are present. The launch guard permits only narrow read-only app/build inspection, so the app record, processed builds, availability, version attachment, and strict validation could not be checked without maintainer-provided release access.
