...
- App Store Connect inspection was unavailable in this run. The Apple developer team setting is empty, and allowed read-only `asc` commands recurse into the command guard because the runner does not export the real `asc` executable. Issue #30 tracks the guard fix. The app record, processed builds, availability, version attachment, and strict validation therefore could not be checked.
